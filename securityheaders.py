...
def evaluate_warn(header, contents):
...
def check_headers(url, follow_redirects = 0):
    ...
    if (res.status >= 300 and res.status < 400  and follow_redirects > 0):
        for header in headers:
            if (header[0] == 'location'):
               return check_headers(header[1], follow_redirects - 1) 
                
    """ Loop through headers and evaluate the risk """
    for header in headers:
        if (header[0] in retval):
            retval[header[0]] = evaluate_warn(header[0], header[1])
